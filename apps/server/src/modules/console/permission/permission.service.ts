...
export class PermissionService extends BaseService<Permission> implements OnModuleInit {
    ...
    async findByCode(code: string): Promise<Partial<Permission>> {
        ...
        if (!permission) {
            throw HttpExceptionFactory.notFound(`权限编码 ${code} 不存在`);
        }
    ...
    async syncApiPermissions(): Promise<{
            ...
            if (dbPermission.apiPath && !currentPermissionCodes.has(dbPermission.code)) {
                ...
                try {
                    ...
                    const deleteResult = await this.dataSource
                        ...
                        .where("permission_id = :permissionId", { permissionId: dbPermission.id })
                        .execute();
        ...
        for (const permission of permissions) {
            ...
            let permissionPrefix: string;
            // 检查是否有前缀（插件权限格式：pluginPackName@code）
            if (permission.code.includes("@")) {
                const permissionCodeSplitRes = permission.code.split("@");
                permissionPrefix = permissionCodeSplitRes[0];
            ...
            if (existingPermission) {
                ...
                existingPermission.pluginPackName = permission.pluginPackName; // 设置插件包名（如果有）
                ...
                await this.create({
                    ...
                    isDeprecated: false,
                    type: permission.type, // 设置权限类型
                    pluginPackName: permission.pluginPackName, // 设置插件包名（如果有）
                });
                added++;
     ...
     * @returns 清理结果
     */
    async cleanupDeprecatedPermissions(): Promise<{ removed: number; menuUpdated: number }> {
        // 获取所有废弃的权限
        ...
        for (const permission of deprecatedPermissions) {
            try {
                ...
                await this.dataSource
                    ...
                    .delete()
                    .from(junctionTableName)
                    .where("permission_id = :permissionId", { permissionId: permission.id })
                    .execute();
                ...
                throw HttpExceptionFactory.internal(`删除废弃权限失败: ${permission.code}`);
            }
        }
