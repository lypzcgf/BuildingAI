...
| 支付  | PayService.createPrepayOrder               | 同充值，仅 `from='coze'` 区分业务源 |
| 字典  | DictService.value('coze\_package\_enable') | 控制上下架                               |
| 状态机 | OrderStatusEnum                            | 复用 Console 枚举                       |
| 错误码 | `PACKAGE_SOLD_OUT` / `ORDER_NOT_FOUND`     | 复用 Console 定义                       |

### 4.5 支付集成细节

**支付接口定义** (`apps/server/src/common/interfaces/pay.interface.ts`):
- 新增 `PayFrom.COZE` 枚举值，用于标识Coze套餐订单支付来源
- `PayOrder` 接口统一处理不同业务源的支付订单

**支付模块配置** (`apps/server/src/modules/web/pay/pay.module.ts`):
- 注入 `CozePackageOrder` 实体，支持Coze套餐订单的支付处理
- 复用现有 `PayService`、`PayconfigService`、`WxPayService` 等支付相关服务

**支付服务实现** (`apps/server/src/modules/web/pay/services/pay.service.ts`):
- `prepay()` 方法支持 `from='coze'` 参数，处理Coze套餐订单预支付
- `getPayResult()` 方法支持查询Coze套餐订单支付状态
- 统一订单金额计算逻辑，支持 `orderAmount` 或 `totalAmount` 字段
- 支付回调处理复用现有微信支付通知机制
...
| 文件   | 路径                                                             | 说明                          | 备注                                                                            |
| ---- | -------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------- |
| 页面组件 | `apps/web/app/profile/personal-rights/coze-package-center.vue` | 个人权益分组下的 Coze 套餐中心页面        | `definePageMeta({ layout: 'setting', inSystem: true, inLinkSelector: true })` |
| 模型类型 | `apps/web/models/coze-package-center.d.ts`                     | 页面与服务的 TS 类型定义              | 与后端 DTO 字段对齐                                                                  |
| 接口服务 | `apps/web/services/web/coze-package-center.ts`                 | 调用 `/web/coze-package/*` 接口 | 传参需带 `from='coze'` 区分业务                                                       |
...
| 文件路径                                                           | 改动性质      | 要点说明                                                                                                                                                                                                  |
| -------------------------------------------------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/web/app/profile/personal-rights/coze-package-center.vue` | 新增        | 路由 `/profile/personal-rights/coze-package-center`；`definePageMeta` 标题 `menu.cozePackageCenter`，复用充值中心交互与轮询逻辑                                                                                          |
| `apps/web/models/coze-package-center.d.ts`                     | 新增        | 对齐 Console DTO，定义 `CenterInfo/OrderParams/OrderInfo/PayResult` 等类型                                                                                                                                    |
| `apps/web/services/web/coze-package-center.ts`                 | 新增        | 接口：`GET /web/coze-package/center`、`POST /web/coze-package/order`、`GET /web/coze-package/orders`；支付复用 `/pay/prepay` 与 `/pay/getPayResult`（传 `from='coze'`）                                             |
